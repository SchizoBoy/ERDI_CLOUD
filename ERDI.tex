...
\begin{document}
  ...
  \section{Anticipation et prévention des risques}
      L'identification des riques liés à une installation dans le cloud est
      importante car elle permet de pouvoir réfléchir à des moyens d'anticiper
      les attaques et de les prévenir. Pour cela, on se penche sur différents 
      aspects sources de risques, et par conséquent nécessaire à sécuriser.

    \subsection{Un aspect technique}
      ...
      le côté interne aux systèmes et les procédures à mettre en place au 
      niveau de la configuration système et logicielle. \\

      Dans un premier temps, il est bien évidemment nécessaire de limiter les 
      accès à la plateforme. Que ce soit en passant par des mots de passes 
      sécurisés ou éventuellement en optant pour une authentification à double
      facteurs. L'accès au cloud est crucial puisque contrairement à un système
      plus classique, le cloud étant une seule machine qui plus est connue de
      tous, les points d'entrées d'éventuelles attaques sont centralisés. \\

      Toujours dans cette même idée de contrôle d'accès il est judicieux de 
      détenir un pare-feux afin de filtrer les connexions sur les serveurs 
      distants. Cela ne s'applique pas à tous les types de clouds car dans
      le cas d'un cloud SaaS, ce côté là sera géré par le fournisseur de 
      services. \\

      Étant dans un environnement cloud, on peut aussi utiliser un CASB ou
      Cloud Access Security Broker. Il s'agit d'une solution d'ensemble 
      permettant de recenser l'ensemble des applications du cloud afin de
      répertorier et classifier les potentielles vulnérabilités.

    \subsection{Un aspect physique}
      ...
      ce but comme la certification ISO 9000 ou SAS 70 par exemple. \\
      ...
      plus simplement des coupures de courant en prévoyant une redondance des
      données par des sauvegardes régulières chiffrées et stockées sur d'autres 
      infrastructures. Dans le cadre d'un service qui outrepasse le simple
      stockage de données, il est aussi nécessaire de prévoir une redirection 
      du service vers une autre plateforme fonctionnelle afin de garantir qu'il
      n'y ait aucune interruption de service.
