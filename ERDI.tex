...
\begin{document}
  ...
  \section{Introduction}
    ...
    encore OVH proposent de louer des serveurs afin que les particuliers comme
    les entreprises puissent héberger des services en ligne. On distingue ici
    quatres modèles de cloud proposé par les fournisseurs, le modèle sur site,
  ...
  \section{Les failles de sécurité qui peuvent mener à un vol de données}
    ...
    \subsection{Différentes erreurs humaines}
      ...
      données sur un cloud proviennent d'une erreur humaine, une mauvaise
      ...
      \subsubsection{Mauvaise gestion du contrôle d'accès}
        ...
        service. Parfois un trop grand nombre de personne utilisant le service
        peut avoir accès à des données sensibles, par exemple au sein d'une
    ...
    \subsection{La gestion de la mémoire dans le cloud}
      ...
      \begin{figure}[h]
        \centering
        \includegraphics[scale=.5]{img/schema_memoire.png}
    ...
    \subsection{Risques physiques}
      ...
      En effet, il est aussi nécessaire de réfléchir à la protection des centre
      de données. La sécurité des centres est importante car ils contiennent
      l'intégralité des données utilisateurs. On peut imaginer des attaquants
      essayer de pirater physiquement un centre de données.
    ...
    \appendix
      \section{Différents modèles de cloud}
        \begin{figure}[h]
          ...
          \caption{Services gérés par le fournisseur ou par le client en fonction
          du modèle}
        \end{figure}
        \label{modeles}
